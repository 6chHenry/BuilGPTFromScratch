...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        assert len(sd_keys) == len(sd_hf_keys), f'mismatched lengths {len(sd_keys)} != {len(sd_hf_keys)}'
        for k in sd_hf_keys:
            if any(k.endswith(w) for w in transposed):
                 assert sd_hf[k].shape[::-1] == sd[k].shape, f"mismatched shapes {sd_hf[k].shape[::-1]} != {sd[k].shape}"
                 with torch.no_grad():
...
for i in range(50):
    ...
    print(f"step {i} , loss : {loss.item()}, dt: {dt:.2f}ms,tok/sec: {tokens_per_sec:.42f}")
